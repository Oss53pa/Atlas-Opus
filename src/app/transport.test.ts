...
describe('createRepoTransport — rejeu concret', () => {
  ...
  it('pgesActions.create → appelle pgesActions.add et renvoie ok', async () => {
    ...
    const api = {
      siteReports: { add: async () => { throw new Error('nope'); } },
      pgesActions: { add: async (opId: string, input: unknown) => { calls.push({ opId, input }); return {} as never; } },
    };
    const res = await createRepoTransport(api as never)(
      mutation({ entity: 'pgesActions', op: 'create', payload: { operationId: 'op-1', action: 'Arrosage', responsable: 'HSE', echeance: '2026-09-30', indicateur: 'Fréquence' } }),
    );
    expect(res).toEqual({ ok: true });
    expect(calls[0]).toMatchObject({ opId: 'op-1', input: { action: 'Arrosage', responsable: 'HSE', echeance: '2026-09-30', indicateur: 'Fréquence' } });
  });

  it('offerScores.create → appelle offerScores.setScore et renvoie ok', async () => {
    const calls: unknown[] = [];
    const api = {
      siteReports: { add: async () => { throw new Error('nope'); } },
      offerScores: { setScore: async (opId: string, input: unknown) => { calls.push({ opId, input }); return {} as never; } },
    };
    const res = await createRepoTransport(api as never)(
      mutation({ entity: 'offerScores', op: 'create', payload: { operationId: 'op-1', offerId: 'of-1', criteriaId: 'ec-1', rawScore: 85, weightedScore: 34 } }),
    );
    expect(res).toEqual({ ok: true });
    expect(calls[0]).toMatchObject({ opId: 'op-1', input: { offerId: 'of-1', criteriaId: 'ec-1', rawScore: 85, weightedScore: 34 } });
  });

  it('evaluationCriteria.create → appelle evaluationCriteria.add et renvoie ok', async () => {
    const calls: unknown[] = [];
    const api = {
      siteReports: { add: async () => { throw new Error('nope'); } },
      evaluationCriteria: { add: async (opId: string, input: unknown) => { calls.push({ opId, input }); return {} as never; } },
    };
    const res = await createRepoTransport(api as never)(
      mutation({ entity: 'evaluationCriteria', op: 'create', payload: { operationId: 'op-1', label: 'Prix', type: 'financier', weight: 0.45 } }),
    );
    expect(res).toEqual({ ok: true });
    expect(calls[0]).toMatchObject({ opId: 'op-1', input: { label: 'Prix', type: 'financier', weight: 0.45 } });
  });
